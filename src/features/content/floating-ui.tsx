...
import { getFloatingButtonPosition, getPreferredExportFormat, setFloatingButtonPosition, setPreferredExportFormat } from '../../lib/storage';
...
function FloatingApp(props: {
  ...
  const [position, setPosition] = React.useState<FloatingButtonPosition | null>(null);
  ...
  React.useEffect(() => {
    ...
    const initializePosition = async () => {
      const saved = await getFloatingButtonPosition(props.positionScope).catch(() => null);
      if (cancelled) return;
      ...
      if (!saved || saved.x !== nextPosition.x || saved.y !== nextPosition.y) {
        void setFloatingButtonPosition(props.positionScope, nextPosition).catch(() => undefined);
      }
    };
  ...
  const handlePointerDown = React.useCallback((event: React.PointerEvent<HTMLButtonElement>) => {
    ...
    const currentPosition = positionRef.current ?? buildDefaultPosition(viewport, getButtonMetrics(buttonRef.current));
    ...
    const onPointerMove = (moveEvent: PointerEvent) => {
      ...
      if (!dragState.dragging && Math.hypot(dx, dy) < DRAG_THRESHOLD_PX) return;
    ...
    const finishDrag = (endEvent: PointerEvent) => {
      ...
      void setFloatingButtonPosition(props.positionScope, positionRef.current).catch(() => undefined);
    };
  ...
  const panelPlacement = getPanelPlacement(position, viewport, getButtonMetrics(buttonRef.current));
      ...
      {open && (
        <div
          style={{
            ...
            ...(panelPlacement.horizontal === 'start' ? { left: 0 } : { right: 0 }),
          }}
        >
          <ActionPanel
      ...
      {showButton && (
        <button
          ...
          style={{
            ...
            padding: '10px 14px',
            borderRadius: 999,
            border: open ? `${ACTIVE_BORDER_PX}px solid ${ACTIVE_ACCENT}` : '1px solid rgba(15,23,42,0.14)',
        ...
        >
          <LogoIcon size={18} />
          <span style={{ fontWeight: 600, color: open ? '#fff' : ACTIVE_ACCENT }}>Export</span>
        </button>
      )}
    </div>
  );
...
function buildDefaultPosition(viewport: { width: number; height: number }, metrics: { width: number; height: number }): FloatingButtonPosition {
...
function getPanelPlacement(
  ...
  return {
    vertical: centerY <= viewport.height / 2 ? 'below' as const : 'above' as const,
    horizontal: centerX <= viewport.width / 2 ? 'start' as const : 'end' as const,
  };
}
