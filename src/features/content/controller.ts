...
import { getLastClaudeOrgId, getShowFloatingButton, setLastClaudeOrgId, setShowFloatingButton } from '../../lib/storage';
import type {
...
function looksGenericProviderTitle(value: string): boolean {
  ...
  return normalized === 'chatgpt' || normalized === 'claude' || normalized === 'project';
}
...
function guessProjectName(project: ProjectListing, pageUrl: string): string {
  if (project.projectName && !/^Project-[A-Za-z0-9-]+$/.test(project.projectName)) return project.projectName;
...
function withBetterProjectName(project: ProjectListing): ProjectListing {
  ...
  return nextName === project.projectName ? project : { ...project, projectName: nextName };
}

function firstNonGenericTitleSegment(value: string): string | null {
  return titleSegments(value).find((segment) => !looksGenericProviderTitle(segment)) ?? null;
}
...
function normalizeClaudeProjectChatTitle(value: string): string {
  const normalized = value
    ...
    .filter(Boolean)
    .filter((segment) => !/^(last message\b|updated\b|edited\b|today\b|yesterday\b|\d+\s+(?:minutes?|hours?|days?|weeks?|months?)\s+ago\b)/i.test(segment));
...
function isProbablyProjectChatTitle(value: string, projectTitle: string | null): boolean {
  ...
  if (/^(new chat|start chat|untitled(?:-chat)?|home|projects?)$/i.test(trimmed)) return false;
...
function scrapeClaudeProjectFromDom(): ProjectListing | null {
  if (inferProvider(currentUrl()) !== 'claude' || inferPageKind(currentUrl()) !== 'project') return null;
...
function currentClaudeOrgIdFromContext(): string | null {
  ...
  return extractClaudeOrgIdFromUrl(observed.find((url) => extractClaudeOrgIdFromUrl(url) !== null) ?? '') || lastClaudeOrgId || null;
}

function buildClaudeListFilesUrl(orgId: string, conversationId: string): string {
  return `https://claude.ai/api/organizations/${orgId}/conversations/${conversationId}/wiggle/list-files?prefix=`;
}

function buildClaudeDownloadFileUrl(orgId: string, conversationId: string, path: string): string {
...
function parseClaudeListFiles(text: string): ClaudeListFileRecord[] {
  ...
  const metadata = Array.isArray((data as any).files_metadata) ? (data as any).files_metadata : [];
  return metadata
    .filter((entry: any) => typeof entry?.path === 'string')
    .map((entry: any) => {
      const path = String(entry.path);
      const filename = typeof entry?.custom_metadata?.filename === 'string' && entry.custom_metadata.filename.trim()
        ? entry.custom_metadata.filename.trim()
        : path.split('/').filter(Boolean).pop() || 'untitled';
      return {
        path,
        filename,
        contentType: typeof entry?.content_type === 'string' ? entry.content_type : undefined,
        createdAt: typeof entry?.created_at === 'string' ? entry.created_at : undefined,
        relativeOutputPath: path.startsWith('/mnt/user-data/outputs/') ? path.slice('/mnt/user-data/outputs/'.length) : undefined,
...
function extractClaudeDownloadedDocumentMarkdown(text: string): { markdown: string; mimeType?: string } | null {
  const direct = (() => {
    try {
  ...
  if (direct && typeof direct === 'object' && (direct as any).content && typeof (direct as any).content === 'object') {
...
async function maybeEnrichClaudeConversationWithGeneratedDocuments(conversation: Conversation, allowNetworkFallback = true): Promise<Conversation> {
  if (!allowNetworkFallback) return conversation;
  if (conversation.provider !== 'claude') return conversation;
  if (conversation.generatedDocuments && conversation.generatedDocuments.length > 0) return conversation;

  const orgId = currentClaudeOrgIdFromContext();
  if (!orgId || !conversation.id) return conversation;

  const listFilesResult = await pageFetch(buildClaudeListFilesUrl(orgId, conversation.id)).catch(() => null);
  if (!listFilesResult?.ok || !listFilesResult.text.trim()) return conversation;

  const listedFiles = parseClaudeListFiles(listFilesResult.text).filter(isClaudeGeneratedDocumentFile);
  if (listedFiles.length === 0) return conversation;

  const generatedDocuments: GeneratedDocument[] = [];
  for (const file of listedFiles) {
    const downloadResult = await pageFetch(buildClaudeDownloadFileUrl(orgId, conversation.id, file.path)).catch(() => null);
    if (!downloadResult?.ok || !downloadResult.text.trim()) continue;

    const extracted = extractClaudeDownloadedDocumentMarkdown(downloadResult.text);
...
function handleRawCapture(message: RawCaptureMessage) {
  ...
  const conversation = parseConversation(message.url, message.text, currentUrl());
...
function getActiveConversationForPage(): Conversation | null {
  ...
  if (currentChatId && conversation.id !== currentChatId) {
    const sourceChatId = conversation.sourceUrl ? extractCurrentChatId(conversation.sourceUrl) : null;
...
async function ensureActiveConversationForPage(allowNetworkFallback = true): Promise<Conversation | null> {
  let conversation = getActiveConversationForPage();
  if (conversation) {
    const enriched = await maybeEnrichClaudeConversationWithGeneratedDocuments(conversation, allowNetworkFallback);
  ...
  if (inferPageKind(currentUrl()) === 'chat') {
    ...
    const apiUrl = provider === 'chatgpt'
      ...
      : buildClaudeCurrentConversationApiUrl(currentUrl(), observed, lastClaudeOrgId);
    if (allowNetworkFallback && apiUrl) {
      ...
      if (result?.ok && result.text.trim()) {
        ...
        if (parsed) {
          latestConversation = await maybeEnrichClaudeConversationWithGeneratedDocuments(parsed, allowNetworkFallback);
        }
      }
    }
  }
...
async function ensureActiveProjectData(): Promise<ProjectListing | null> {
  let project = getActiveProjectForPage();
  if (project) return project;

  if (inferPageKind(currentUrl()) === 'project') {
    const provider = inferProvider(currentUrl());
    const observed = collectObservedApiUrls();
    const observedOrgId = extractClaudeOrgIdFromUrl(observed.find((url) => extractClaudeOrgIdFromUrl(url) !== null) ?? '');
    ...
    const listingUrl = buildCurrentProjectListingUrl(currentUrl(), observed, lastClaudeOrgId);
    if (listingUrl) {
...
export async function initializeController(): Promise<void> {
  ...
  browser.runtime.onMessage.addListener((message: ProjectExportProgressMessage) => {
  ...
  const observedOrgId = extractClaudeOrgIdFromUrl(collectObservedApiUrls().find((url) => extractClaudeOrgIdFromUrl(url) !== null) ?? '');
  if (observedOrgId && observedOrgId !== lastClaudeOrgId) {
    lastClaudeOrgId = observedOrgId;
    void setLastClaudeOrgId(observedOrgId).catch(() => undefined);
  }
...
export async function setFloatingButtonVisible(value: boolean): Promise<UiContext> {
...
export async function getActiveConversationData(allowNetworkFallback = true): Promise<Conversation | null> {
  return ensureActiveConversationForPage(allowNetworkFallback);
}

export async function getActiveProjectData(_allowNetworkFallback = true): Promise<ProjectListing | null> {
...
export async function exportChat(format: ExportFormat, target: 'file' | 'clipboard', selectedMessageIds?: string[]): Promise<void> {
  ...
  for (const canvas of rendered.canvases) {
    if (format === 'html') {
      if (canvas.htmlRelativePath && canvas.html) zip.file(canvas.htmlRelativePath, canvas.html);
      continue;
    }
    if (canvas.markdownRelativePath) zip.file(canvas.markdownRelativePath, canvas.markdown);
  }
...
export async function exportProject(format: ExportFormat): Promise<void> {
  ...
  if (!activeProject && inferPageKind(currentUrl()) === 'project' && inferProvider(currentUrl()) !== 'claude') {
    ...
    } satisfies CollectProjectListingMessage) as { ok?: boolean; project?: ProjectListing; error?: string };
  ...
  setProjectStatus(`Preparing: ${capitalizeWord(activeProject.provider)} - ${activeProject.projectName} - ${activeProject.chats.length} chats`);
  try {
    const result = await browser.runtime.sendMessage({
      type: 'COLLECT_PROJECT_CONVERSATIONS',
      project: activeProject,
      currentProjectPageUrl: currentUrl(),
    } satisfies CollectProjectConversationsMessage) as { ok?: boolean; conversations?: Conversation[]; error?: string };
    ...
    if (conversations.length === 0) throw new Error('No project chats could be exported.');

    const zip = new JSZip();
    const folderName = safeFilenamePart(activeProject.projectName);
    const sorted = conversations.slice().sort((a, b) => {
      const ai = activeProject.chats.find((chat) => chat.id === a.id)?.order ?? 0;
      const bi = activeProject.chats.find((chat) => chat.id === b.id)?.order ?? 0;
      return ai - bi;
    });
    ...
    for (const conversation of sorted) {
      ...
      for (const canvas of prepared.canvases) {
        if (format === 'html') {
          if (canvas.htmlRelativePath && canvas.html) zip.file(`${folderName}/${canvas.htmlRelativePath}`, canvas.html);
          continue;
        }
        if (canvas.markdownRelativePath) zip.file(`${folderName}/${canvas.markdownRelativePath}`, canvas.markdown);
      }
    }
