...
export function inferPageKind(urlString: string): PageKind {
	try {
		...
		if (url.hostname === "chatgpt.com") {
			...
			if (
				/\/g-p-[A-Za-z0-9]+/.test(path) ||
				/^\/g\/g-p-[A-Za-z0-9]+$/.test(path)
			)
				return "project";
			if (path === "/" || /^\/gpts/.test(path) || /^\/library/.test(path))
				return "unsupported";
			return "unknown";
		}
		...
		if (url.hostname === "claude.ai") {
			...
			if (
				/\/(project|projects)\/[A-Za-z0-9-]+\/(chat|chats)\/[A-Za-z0-9-]+$/.test(
					path,
				)
			)
				return "chat";
			if (/\/(project|projects)\/[A-Za-z0-9-]+$/.test(path)) return "project";
			if (path === "/" || /^\/new/.test(path) || /^\/settings/.test(path))
				return "unsupported";
			return "unknown";
		}
...
export function extractCurrentChatId(urlString: string): string | null {
	try {
		...
		const claudeProject = path.match(
			/\/(?:project|projects)\/[A-Za-z0-9-]+\/(?:chat|chats)\/([A-Za-z0-9-]+)$/,
		);
...
export function buildCurrentProjectListingUrl(
	currentProjectPageUrl: string,
	observedApiUrls: string[],
	fallbackClaudeOrgId?: string | null,
): string | null {
	...
	if (provider === "claude") {
		...
		const projectId = extractCurrentProjectId(currentProjectPageUrl);
		const orgId =
			extractClaudeOrgIdFromUrls(observedApiUrls) ??
			fallbackClaudeOrgId ??
			null;
...
export function buildClaudeCurrentConversationApiUrl(
	...
	const observed = chatId
		? observedApiUrls.find(
				(url) =>
					isClaudeConversationUrl(url) &&
					url.includes(`/chat_conversations/${chatId}`),
			)
		: null;
	if (observed) return observed;
	const orgId =
		extractClaudeOrgIdFromUrls(observedApiUrls) ?? fallbackClaudeOrgId ?? null;
...
export function buildProjectChatPageUrls(
	project: ProjectListing,
	currentProjectPageUrl: string,
	chatId: string,
): string[] {
	if (project.provider === "chatgpt") {
		const base = currentProjectPageUrl
			.replace(/\/project(?:\?.*)?$/, "")
			.replace(/\/$/, "");
