...
function normalizePositionMap(value: unknown): FloatingButtonPositionsByScope {
	...
	for (const [scope, position] of Object.entries(
...
export async function setPreferredExportFormat(
	value: ExportFormat,
): Promise<void> {
...
export async function setFloatingButtonPosition(
	scope: string,
...
export async function getLastClaudeOrgId(): Promise<string | null> {
	...
	return typeof result[CLAUDE_ORG_ID_KEY] === "string" &&
		result[CLAUDE_ORG_ID_KEY].trim()
		? result[CLAUDE_ORG_ID_KEY]
		: null;
}
