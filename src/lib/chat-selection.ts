...
export function deriveConversationTurns(conversation: Conversation): ConversationTurn[] {
...
export function filterConversationToMessageIds(conversation: Conversation, selectedMessageIds: string[]): Conversation {
  const allowed = new Set(selectedMessageIds);
  return {
    ...conversation,
    messages: conversation.messages.filter((message) => allowed.has(message.id)),
  };
}
...
function buildRolePreview(messages: Message[], role: Message['role']): string {
  ...
  if (roleMessages.length === 0) {
    return role === 'user' ? 'No user prompt in this selection unit.' : 'No assistant reply in this selection unit.';
  }

  const first = summarizeMarkdown(roleMessages[0].markdown, role === 'user' ? 160 : 220);
