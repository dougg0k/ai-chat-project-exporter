function replaceInternalUiTokens(input: string): string {
	return input.replace(
		/([^]+)([\s\S]*?)/g,
		(_match, tokenType: string, rawPayload: string) => {
...
function stripInternalJsonBlocks(input: string): string {
	return input
		.replace(
			/```json\s*\{[\s\S]*?(?:search_query|response_length|image_query|product_query)[\s\S]*?\}\s*```/g,
			"",
		)
		.trim();
}
...
export function cleanVisibleMarkdown(input: string): string {
	return repairSplitHeadings(
		stripInternalJsonBlocks(replaceInternalUiTokens(input)),
	)
		.replace(/\r\n/g, "\n")
		.replace(/\r/g, "\n")
		.replace(/\u0000/g, "")
