...
export function parseClaudeConversation(url: string, text: string, sourceUrl: string): Conversation | null {
  ...
  const messages: Message[] = data.chat_messages
    ...
    .map((msg: any) => {
      const role = msg.sender === 'assistant' ? 'assistant' : msg.sender === 'human' ? 'user' : null;
      if (!role) return null;
      const markdown = Array.isArray(msg.content) ? extractClaudeText(msg.content) : '';
  ...
  return {
    ...
    provider: 'claude',
    title: typeof data.name === 'string' && data.name.trim() ? data.name.trim() : 'Untitled-Chat',
    sourceUrl,
...
export function parseClaudeProject(url: string, text: string): ProjectListing | null {
  if (!url.includes('conversations_v2/')) return null;
  const data = unwrapRecordedOrDirectJson(text);
  if (!Array.isArray(data?.data) || data.data.length === 0) return null;

  const match = url.match(/\/api\/organizations\/([^/]+)\/(?:project|projects)\/([^/]+)\/conversations_v2/i);
  const orgId = match?.[1];
  ...
  const chats: ProjectChatRef[] = data.data
    ...
    .map((item: any, index: number) => ({
      id: item.uuid,
      title: typeof item.name === 'string' && item.name.trim() ? normalizeClaudeProjectChatTitle(item.name) : 'Untitled-Chat',
      order: index,
